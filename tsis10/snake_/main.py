...
class Snake:
    ...
    def drawingSnake(self):
        for block in self.body:  # the for loop that iterate our list of cooredinates of snake elements
            body_rect = pygame.Rect(block.x * cell_size, block.y * cell_size, cell_size,
                                    cell_size)  # creating rectangle of the snake elements
    ...
    def snakeMoving(self):
        if self.eated == True:  # if snake eated the fruit
            ...
            body_copy.insert(0, body_copy[
                0] + direction)  # adding one element at index 0 this element is our head + direction
            self.body = body_copy[:]
...
class Fruit:
    ...
    def randomize(self):
        self.x = random.randint(0,
                                cell_number - 2)  # random cell between 0 - 18, whu 18 because if it will be bigger, the fruit will spawn on the borders
...
class Game:
    ...
    def checkCollision(self):
        if (self.fruit.pos == self.snake.body[
            0]):  # if the coordinates of our snake and fruit will be equal, snake will eat
...
def name_checker(NAMEBOX):
    ...
    if cnt > 0:
        print(
            f'User with this name is already exist, the level of this user is: {int(row[2]) // 3}, please enter another name: \n')
    else:
        start_the_game()
    ...
    while not done:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_i:
                cur.execute(
                    f''' INSERT INTO postgres.public.snake_scores("user", "user_score") VALUES( '{user_name}', '{game.score}'); ''')
